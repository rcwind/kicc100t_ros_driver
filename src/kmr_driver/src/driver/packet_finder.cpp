/**
 * @file /kmr_driver/src/driver/packet_finder.cpp
 *
...
#include <sstream>
#include "../../include/kmr_driver/packet_handler/packet_finder.hpp"
...
unsigned int PacketFinderBase::numberOfDataToRead()
{
  ...
  switch (state)
  {
    ...
    case waitingForPayloadToEtx:
	      //size_etx=0,size函数求的大小是压入栈中的元素个数，初始化的时候没有压数据到ETX里
	      num = size_payload + size_etx + size_checksum_field - (buffer.size() - size_stx - size_length_field);
...
bool PacketFinderBase::update(const unsigned char * incoming, unsigned int numberOfIncoming)
{
  ...
  {
    case waitingForStx:
	// 判断头0xaa 0x55,分两次判断，不是一次完成，两次都读一个字节来判断
      if (WaitForStx(incoming[0]))
      {
        if (size_length_field)// kmr协议固定为一个字节的length,位于header字节之后
        {
		// 接下来读length字段
          state = waitingForPayloadSize; // kmrbot
        }
        else
        {
